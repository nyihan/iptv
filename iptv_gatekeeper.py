...
# သင့် Supabase Project URL နှင့် လူကြီးမင်းပေးပို့ထားသော API Key အမှန် ဖြစ်ပါသည်
SUPABASE_URL = "https://mykfodrelbkdsmednyka.supabase.co"  
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im15a2ZvZHJlbGJrZHNtZWRueWthIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2MjAxMzUsImV4cCI6MjA5NjE5NjEzNX0.40noW7Max6dumYNBmf5CYxYjmWaB3oZIKT4e8MunfBE"  
BUCKET_NAME = "iptv"  
...
def admin_panel():
    ...
    <body class="bg-gray-955 text-gray-100 min-h-screen p-4 md:p-6">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="bg-red-950/80 border border-red-800 text-red-200 p-5 rounded-2xl mb-8 space-y-2 shadow-xl">
                ...
                <p class="text-xs text-gray-400"><strong>ဖြေရှင်းရန်:</strong> သင့် ကုဒ်ဖိုင်ထဲရှိ <code>SUPABASE_KEY</code> နေရာတွင် 'sb_secret_' စသည့် Key အစား <strong>'eyJ'</strong> နှင့်စတင်သည့် <strong>Anon Public Key (သို့မဟုတ်) Service Role Key</strong> အရှည်ကြီးကို ပြောင်းလဲထည့်သွင်းပေးပါရန်။</p>
            </div>
            {% endif %}
            ...
            <div class="bg-gray-900 p-6 rounded-2xl shadow-xl border border-gray-800 mb-8 flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <form action="/admin/create" method="POST" class="flex gap-2">
                    <button type="submit" {% if db_error %}disabled class="bg-gray-700 text-gray-400 font-bold py-3 px-6 rounded-lg cursor-not-allowed"{% else %}class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg shadow-lg hover:shadow-blue-900/20 transition duration-200"{% endif %}>
